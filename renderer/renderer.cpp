...
/*
        ...
        Linear tiled images:
                These are stored as is and can be copied directly to. But due to
   the linear nature they're not a good match for GPUs and format and feature
   support is very limited. It's not advised to use linear tiled images for
   anything else than copying from host to GPU if buffer copies are not an
   option. Linear tiling is thus only implemented for learning purposes, one
   should always prefer optimal tiled image.
   ...
   liner tiled images) and always require some sort of data copy, either from a
   buffer or	a linear tiled image.
...
void Renderer::prepare() {
  ...
  if (settings.overlay) {
    ...
    ui.prepareResources();
    ui.preparePipeline(pipelineCache, renderPass, swapChain.colorFormat,
                       depthFormat);
  }
...
void Renderer::buildCommandBuffer() {
  ...
  vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          pipelineLayout, 0, 1, &descriptorSets[currentBuffer],
                          0, nullptr);
...
void Renderer::drawUI(const VkCommandBuffer commandBuffer) {
  if (settings.overlay && ui.visible) {
    const VkViewport viewport{.width = (float)width,
                              .height = (float)height,
                              .minDepth = 0.0f,
                              .maxDepth = 1.0f};
...
Renderer::loadShader(std::string fileName, VkShaderStageFlagBits stage) {
...
void Renderer::prepareFrame(bool waitForFence) {
  ...
  VkResult result = swapChain.acquireNextImage(
      presentCompleteSemaphores[currentBuffer], currentImageIndex);
...
Renderer::Renderer() {
  ...
  commandLineParser.add("validationlogfile", {"-vl", "--validationlogfile"}, 0,
                        "Log validation messages to a textfile");
  ...
  commandLineParser.add("shaders", {"-s", "--shaders"}, 1,
                        "Select shader type to use (gls, hlsl or slang)");
  ...
  commandLineParser.add(
      "resourcepath", {"-rp", "--resourcepath"}, 1,
  ...
  if (commandLineParser.isSet("resourcepath")) {
    vks::tools::resourcePath =
        commandLineParser.getValueAsString("resourcepath", "");
  }
...
void Renderer::setupDPIAwareness() {
  typedef HRESULT *(__stdcall *
                    SetProcessDpiAwarenessFunc)(PROCESS_DPI_AWARENESS);
...
void Renderer::handleMessages(HWND hWnd, UINT uMsg, WPARAM wParam,
                              LPARAM lParam) {
  switch (uMsg) {
  case WM_CLOSE:
    prepared = false;
...
void Renderer::setupFrameBuffer() {
  ...
  for (uint32_t i = 0; i < frameBuffers.size(); i++) {
    const VkImageView attachments[2] = {swapChain.imageViews[i],
                                        depthStencil.view};
...
void Renderer::OnHandleMessage(HWND hWnd, UINT uMsg, WPARAM wParam,
                               LPARAM lParam) {}
...
void Renderer::DrawTextureRec(Texture texture, Rectangle rectangle,
                              glm::vec2 position, Color tint) {
  ...
  uint32_t textureVerticesCount = textureVertices.size();

  // Setup indices
  std::vector<uint32_t> indices = {
      textureVerticesCount + 0, textureVerticesCount + 1,
      textureVerticesCount + 2, textureVerticesCount + 2,
      textureVerticesCount + 3, textureVerticesCount + 0};

  textureIndices.insert(textureIndices.end(), indices.begin(), indices.end());
  textureVertices.insert(textureVertices.end(), vertices.begin(),
                         vertices.end());
}
...
void Renderer::setupDescriptors() {
  ...
  for (auto i = 0; i < uniformBuffers.size(); i++) {
    ...
    std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
        ...
        //  Note that unlike the uniform buffer, which is written by the CPU and
        //  the GPU, the image (texture) is a static resource
...
void Renderer::preparePipelines() {
  ...
  std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
      vks::initializers::vertexInputAttributeDescription(
          0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(TextureVertex, pos)),
      vks::initializers::vertexInputAttributeDescription(
          0, 1, VK_FORMAT_R32G32_SFLOAT, offsetof(TextureVertex, uv)),
      vks::initializers::vertexInputAttributeDescription(
          0, 2, VK_FORMAT_R32G32B32_SFLOAT, offsetof(TextureVertex, normal)),
  };
...
void Renderer::EndDrawing() {
  if (prepared && !IsIconic(window)) {
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        &stagingBuffers.indices, textureIndices.size() * sizeof(uint32_t),
        textureIndices.data()));
