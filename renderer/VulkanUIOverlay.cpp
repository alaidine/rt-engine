...
void UIOverlay::prepareResources() {
  ...
  VkImageCreateInfo imageInfo{.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO,
                              ...
                              .extent = {.width = (uint32_t)texWidth,
  ...
  VkBufferImageCopy bufferCopyRegion{
      .imageSubresource = {.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
                           .layerCount = 1},
      .imageExtent = {.width = (uint32_t)texWidth,
                      .height = (uint32_t)texHeight,
                      .depth = 1}};
  ...
  VkWriteDescriptorSet writeDescriptorSets =
      vks::initializers::writeDescriptorSet(
          ...
          &fontDescriptor);
  vkUpdateDescriptorSets(device->logicalDevice, 1, &writeDescriptorSets, 0,
                         nullptr);
...
void UIOverlay::preparePipeline(const VkPipelineCache pipelineCache,
  ...
  VkPushConstantRange pushConstantRange{
      .stageFlags = VK_SHADER_STAGE_VERTEX_BIT, .size = sizeof(PushConstBlock)};
...
void UIOverlay::update(uint32_t currentBuffer) {
  ...
  vertexBufferSize =
      ((vertexBufferSize + chunkSize - 1) / chunkSize) * chunkSize;
...
void UIOverlay::draw(const VkCommandBuffer commandBuffer,
  ...
  vkCmdBindVertexBuffers(commandBuffer, 0, 1,
                         &buffers[currentBuffer].vertexBuffer.buffer, offsets);
  vkCmdBindIndexBuffer(commandBuffer, buffers[currentBuffer].indexBuffer.buffer,
    ...
    for (int32_t j = 0; j < cmd_list->CmdBuffer.Size; j++) {
      ...
      VkRect2D scissorRect{
          .offset = {.x = std::max((int32_t)(pcmd->ClipRect.x), 0),
                     .y = std::max((int32_t)(pcmd->ClipRect.y), 0)},
