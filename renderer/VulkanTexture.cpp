 ...
 *
 */
void Texture2D::loadFromFile(std::string filename, VkFormat format,
  ...
  VkMemoryAllocateInfo memAllocInfo{
      ...
      .memoryTypeIndex = device->getMemoryType(
  ...
  VkSamplerCreateInfo samplerCreateInfo{
      ...
      .maxAnisotropy = device->enabledFeatures.samplerAnisotropy
...
void Texture2D::fromBuffer(void *buffer, VkDeviceSize bufferSize,
                           VkFormat format, uint32_t texWidth,
                           uint32_t texHeight, vks::VulkanDevice *device,
 ...
 *
 */
void Texture2DArray::loadFromFile(std::string filename, VkFormat format,
                                  vks::VulkanDevice *device, VkQueue copyQueue,
                                  VkImageUsageFlags imageUsageFlags,
                                  VkImageLayout imageLayout) {
  ktxTexture *ktxTexture;
 ...
 *
 */
void TextureCubeMap::loadFromFile(std::string filename, VkFormat format,
                                  vks::VulkanDevice *device, VkQueue copyQueue,
                                  VkImageUsageFlags imageUsageFlags,
                                  VkImageLayout imageLayout) {
  ktxTexture *ktxTexture;
    ...
    for (uint32_t level = 0; level < mipLevels; level++) {
      ...
      VkBufferImageCopy bufferCopyRegion{
          ...
          .imageSubresource = {.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
                               ...
                               .baseArrayLayer = face,
                               .layerCount = 1},
          .imageExtent = {.width = ktxTexture->baseWidth >> level,
                          .height = ktxTexture->baseHeight >> level,
                          .depth = 1},
      };
      bufferCopyRegions.push_back(bufferCopyRegion);
